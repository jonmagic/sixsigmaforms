...
module Autologger
  def self.append_features(base)
    ...
    base.before_update do |model|
      unless model.class.to_s.humanize == 'Log'
        old_obj = model.class.find_by_id(model.id)
        Log.create(:log_type => "update:#{model.class.to_s.humanize}", :data => {:old_attributes => old_obj.attributes.changed_values(model.attributes), :new_attributes => model.attributes.changed_values(old_obj.attributes)}, :object => old_obj, :agent => Thread.current['user'])
      end
    end

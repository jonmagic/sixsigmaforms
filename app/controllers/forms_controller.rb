class FormsController < ApplicationController
  ...
  def create
    ...
    @form_type_model = FormType.find_by_form_type(@form_type)
    @patient = Patient.find_by_id(params[:patient_id] || (params[:patient].blank? ? nil : params[:patient][:id])) || Patient.new
    @form = current_user.doctor.form(@form_type).new
    ...
    if @patient.create_or_update
      if @form.create_or_update
        @form.form_instance = FormInstance.new(:user_id => current_user.id, :doctor_id => current_user.doctor.id, :patient_id => @patient.id, :form_type => @form_type_model, :form_type_id => @form_type_model.id, :status => 'draft')
